...
function renderPlanPanel(m, idx) {
  ...
  if (p.status === 'executing' || p.status === 'paused') statsText += ` · ${doneSteps}/${totalSteps}`;
  
  let stepsHtml = '';
  if (p.steps?.length) {
    stepsHtml = `<ol class="plan-step-list">${(p.steps || []).map((s, si) => {
      const editBtns = p.status === 'pending_approval' ? `
      ...
      if (s.toolCalls && s.toolCalls.length) {
        toolCallsHtml = `<div class="plan-tool-calls">${s.toolCalls.map(tc => {
  ...
  if (p.analysis || p.steps?.length) {
    planningHtml = `
      <div class="plan-section planning">
        <div class="plan-section-header">📋 任务规划 ${statusBadge}</div>
        <div class="plan-section-body">
          ...
          ${totalSteps && (p.status === 'executing' || p.status === 'completed' || p.status === 'paused') ? `<div class="plan-progress-bar"><div class="plan-progress-fill" style="width:${progressPct}%;"></div></div>` : ''}
          ${stepsHtml}
        </div>
      </div>`;
  }
          ...
          ${p.reviewTurns.map(t => {
            ...
            return `
              <div style="padding:8px;background:var(--bg-input);border-radius:6px;margin:6px 0;font-size:13px;">
                ...
                ${t.revised_steps ? `<div style="font-size:12px;color:var(--text-secondary);margin-top:4px;">✏️ 已应用改进计划</div>` : ''}
              </div>`;
          }).join('')}
        </div>
      </div>`;
  ...
  if (p.status === 'pending_approval') {
    ...
    approvalHtml = `
      <div class="plan-approval">
        <div class="plan-approval-hint">${p.status === 'paused' ? '⏸ 执行已暂停' : '❌ 执行中断'}，可以从已完成的步骤继续：</div>
        <div class="plan-approval-btns">
...
function togglePlan() {
  ...
  persistSettings();
  updateSendBtn();
  toast(s.usePlan ? '✓ 已启用 Plan' : '✓ 已关闭 Plan');
}
