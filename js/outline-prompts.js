...
4. 代码修改后必须调用 execute_action 运行最相关的测试、构建、lint、启动检查或最小复现命令。
5. 如果命令失败，必须读取 stdout/stderr，继续修改代码，再次运行验证命令；不要在失败后直接总结为完成。
6. 只有验证命令通过，或因为缺依赖、缺配置、缺权限、环境限制等明确阻塞且已说明原因时，才允许最终总结。
