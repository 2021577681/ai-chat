...
function openRateSettings() {
  ...
  if (!modal) {
    ...
    modal.className = 'modal-mask rate-settings-modal';
    modal.innerHTML = `
      <div class="modal rate-settings-dialog">
        <div class="rate-settings-titlebar">
          <div>
            <h3>请求频率设置</h3>
        ...
        <div class="rate-settings-card rate-settings-note">
          <span>推荐设置</span>
          <strong>最小间隔1-3秒、随机延迟0-2秒、每分钟上限10-20次</strong>
        </div>
        
        <div class="rate-settings-card rate-settings-controls">
        ...
        <div class="rate-settings-section-title"><h4>当前统计</h4></div>
        <div id="rateCurrentStats" class="rate-settings-card rate-current-stats"></div>
...
function updateRateCurrentStats() {
  ...
  el.innerHTML = `
    <div class="rate-stat-row"><span>最近 1 分钟</span><strong>${last1m}<small>次</small></strong></div>
    <div class="rate-stat-row"><span>最近 5 分钟</span><strong>${last5m}<small>次</small></strong></div>
    <div class="rate-stat-row"><span>最近 15 分钟</span><strong>${last15m}<small>次</small></strong></div>
    <div class="rate-stat-row"><span>最近 1 小时</span><strong>${last1h}<small>次</small></strong></div>
    <div class="rate-stat-row"><span>今日累计</span><strong>${_requestLog.todayCount}<small>次</small></strong></div>
    <div class="rate-stat-row rate-stat-total"><span>总累计</span><strong>${_requestLog.totalRequests}<small>次</small></strong></div>
  `;
}
...
function applyRatePreset(key) {
  const presets = {
    ...
    safe: { max: 10, min: 1500, randMin: 1000, randMax: 3000, name: '低调模式' },
